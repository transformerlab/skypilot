...
def _handle_task_cloud_credentials(cloud_credentials: Dict[str, Dict[str, str]]) -> Dict[str, str]:
    """Creates temporary credential files from task-provided credentials.
    
    Args:
        cloud_credentials: Dict of {cloud_name: {credential_key: credential_value}}
                         e.g., {'runpod': {'api_key': 'rpa_...'}}
    
    Returns:
        Dict of {remote_path: local_temp_file_path} for file mounts
    """
    import tempfile
    import toml
    ...
    for cloud_name, creds in cloud_credentials.items():
        if cloud_name.lower() == 'runpod' and 'api_key' in creds:
            # Create temporary config.toml file for RunPod
            temp_dir = tempfile.mkdtemp(prefix=f'skypilot_{cloud_name}_')
            ...
            file_mounts['~/.runpod/config.toml'] = config_path
...
def write_cluster_config(
    ...
    # If task has cloud_credentials, use them to create temporary credential files
    if task and task.cloud_credentials:
        credentials.update(_handle_task_cloud_credentials(task.cloud_credentials))
